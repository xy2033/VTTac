...
class StableDiffusionControlNetPipeline(DiffusionPipeline, TextualInversionLoaderMixin):
    ...
    def __init__(
        ...
        self.image_processor = VaeImageProcessor(vae_scale_factor=self.vae_scale_factor) # 初始化图像预处理器,直接加载diffusionVaeImageProcessor
    ...
    def enable_sequential_cpu_offload(self, gpu_id=0):
        ...
        cpu_offloaded_models = [self.unet, self.text_encoder, self.vae]
        if self.controlnet is not None:
            cpu_offloaded_models.append(self.controlnet)
        for cpu_offloaded_model in cpu_offloaded_models:
    ...
    def check_inputs(
        ...
        # 只有在 ControlNet 存在时才检查 ControlNet 相关的输入
        if self.controlnet is not None:
    ...
    def __call__(
        ...
        # 只有在 ControlNet 存在时才处理 ControlNet 相关逻辑
        controlnet = None
        if self.controlnet is not None:
        ...
        prompt_embeds,ram_encoder_hidden_states = self._encode_prompt(
            ...
            ram_encoder_hidden_states=ram_encoder_hidden_states #xy
        )

        # 4. Prepare image
        # 如果没有 ControlNet，使用 unet 的 dtype
        dtype_for_image = self.unet.dtype if self.controlnet is None else self.controlnet.dtype
        ...
        if start_point == 'noise':
            latents = latents
        elif start_point == 'lr': # LRE Strategy xy报错的地方
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                if self.controlnet is not None:
                    ...
                    # 转换为正确的 dtype
                    dtype_for_residuals = self.unet.dtype
                    down_block_additional_residuals = [
                        sample.to(dtype=dtype_for_residuals) for sample in down_block_res_samples
                    ]
                    mid_block_additional_residual = mid_block_res_sample.to(dtype=dtype_for_residuals)

                if h*w<=tile_size*tile_size: # 不需要分块处理
                    # 直接使用UNet预测噪声
                    ...
                    for row in range(grid_rows):
                        for col in range(grid_cols):
                            ...
                            if len(input_list) == batch_size or col == grid_cols-1:
                                ...
                                # 如果使用 ControlNet，为每个 tile 单独调用
                                tile_down_block_additional_residuals = None
                                tile_mid_block_additional_residual = None
                                
                                if self.controlnet is not None:
                                    cond_list_t = torch.cat(cond_list, dim=0)
                                    img_list_t = torch.cat(img_list, dim=0)
                                    
                                    tile_down_block_res_samples, tile_mid_block_res_sample = self.controlnet(
                                        cond_list_t,
                                        t,
                                        encoder_hidden_states=controlnet_prompt_embeds,
                                    ...
                                    if guess_mode and do_classifier_free_guidance:
                                        tile_down_block_res_samples = [torch.cat([torch.zeros_like(d), d]) for d in tile_down_block_res_samples]
                                        tile_mid_block_res_sample = torch.cat([torch.zeros_like(tile_mid_block_res_sample), tile_mid_block_res_sample])
                                    
                                    dtype_for_residuals = self.unet.dtype
                                    tile_down_block_additional_residuals = [
                                        sample.to(dtype=dtype_for_residuals) for sample in tile_down_block_res_samples
                                    ]
                                    tile_mid_block_additional_residual = tile_mid_block_res_sample.to(dtype=dtype_for_residuals)
                                ...
                                model_out = self.unet(
                                    ...
                                    return_dict=False,
                                    image_encoder_hidden_states=ram_encoder_hidden_states,
                                    down_block_additional_residuals=tile_down_block_additional_residuals,
                                    mid_block_additional_residual=tile_mid_block_additional_residual,
                                )[0]
